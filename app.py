...
def generate_and_save_report(user_id: int):
    ...
    try:
        with conn.cursor() as cur:
            ...
            if result:
                ...
                if len(prompt) == 0:
                    cur.execute("INSERT INTO jsol_logarchive_report (USER_IDX, REPORT_USER_TYPE) VALUES (%s, %s)", (user_id, ','.join(result)))
                elif len(prompt) == 1:
                    response = model.generate_content('\n'.join(prompt))
                    #print(response.text)
                    report = response.text
                    cur.execute("INSERT INTO jsol_logarchive_report (USER_IDX, REPORT_USER_TYPE, AI_REPORT_COMMENT) VALUES (%s, %s, %s)", (user_id, ','.join(result), report))
                elif len(prompt) >= 2:
                ...
                conn.commit()
                print(f"report insert 완료 (id: {user_id}, type: {result})")
...
async def generate_report(user_id: int, background_tasks: BackgroundTasks):
    background_tasks.add_task(generate_and_save_report, user_id)
