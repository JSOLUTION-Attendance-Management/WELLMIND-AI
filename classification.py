...
cur.execute(query, (userinfo_idx,))
data = cur.fetchall()
...
for element in result:
    if element == '조퇴 빈발형':
        ...
        queryD = "select user_is_long, user_hobby from jsol_userinfo where userinfo_idx=%s"
        cur.execute(queryD, (userinfo_idx))
        ...
        if element == '지각 빈발형':
            ...
            # 지각 분석
            late_arrivals = arrival_df[arrival_df['time'] >= time(9,0)]
            ...
            if details[0][0] == b'\x00':
               type = "단거리 통근 지각빈발형"
               prompt.append(f"너는 직원의 출퇴근시간을 통해 웰니스 리포트를 작성하는 주체야. 이 직원은 {type}이고, 총 기록 일수: {total_days}일, 평균 출근 시간: {average_time.strftime('%H:%M')}, 지각 기준 시각을 9시 이후로 했을때 지각률: {late_rate:.2f}%, 출근 시간 분포:{time_distribution}, 요일별 평균 출근 시간:\n{weekday_avg}, 월별 평균 출근 시간:\n{monthly_avg}\n이야. (데이터에서 유의미한 결론을 도출해서 작성해줘.) 원인: 시간 관리 문제, 아침 루틴 부재 등등, 해결책: 시간 관리, 아침 루틴(규칙적인 수면시간, 식습관) 만들기 등 이고, 이를 활용하여 (출근시간 및 지각패턴 분석->원인 추측 및 건강 리스크->권장사항 및 해결책) 순서로 리포트를 줄글로 작성해줘.")
            if details[0][0] == b'\x01':
               type = "장거리 통근 지각빈발형"
               hobby = details[0][1]
            ...
            type = "야근빈발형"
            prompt.append(f"너는 직원의 근무시간을 통해 웰니스 리포트를 작성하는 주체야. 이 직원은 {type}이고, 총 기록 일수: {normalAL}일, 야근 일수: {overtime_days}일, 야근 비율: {overtime_rate:.2f}%, 평균 초과 근무 시간: {average_overtime_str}이야. (야근 일수 통계 분석->건강 리스크->권장사항) 순서로 리포트를 줄글로 작성해줘.")
            ...
            type = "출장빈발형"
            prompt.append(f"너는 직원의 근무 환경을 통해 웰니스 리포트를 작성하는 주체야. 이 직원은 {type}이고, 총 근무 일수: {total_days}일, 출장 일수: {business_days}일이야. (출장 일수 통계->건강 리스크->권장사항) 순서로 리포트를 줄글로 작성해줘.")
...
if len(prompt) == 1:
    ...
    response = model.generate_content('해당하는 유형들에 대한 복합적인 리포트를 정리해서 작성해줘.' + ' '.join(prompt))
    print(response.text)